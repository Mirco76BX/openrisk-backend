...
async def search(self, company_name: str) -> list:
    headers = {"User-Agent": "Mozilla/5.0 (compatible; OpenRiskAI/1.0)"}
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            resp = await client.get(
                self.BASE_URL + "/pub/de/native?func=json_sesuche&suchtext=" + company_name + "&kategorie=JA",
                headers=headers
            )
            if resp.status_code == 200:
                return self._parse_search_results(resp.text)
        except Exception as e:
            print("Suche Fehler: " + str(e))
    return []
...
@app.get(”/”)
async def root():
return {“status”: “OpenRisk AI Backend laeuft”, “version”: “1.0.0”}
